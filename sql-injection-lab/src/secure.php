...
if (!empty($username) || !empty($password)) {
    // SICUREZZA: Uso di prepared statement per prevenire SQL injection
    $stmt = $conn->prepare("SELECT id, username, email, created_at FROM users WHERE username = ? AND password = ?");
    
    if ($stmt) {
        // Bind dei parametri - i valori vengono automaticamente escapati
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                // Mostra solo campi sicuri (non password)
        ...
        echo '<div class="error">Errore nella preparazione della query: ' . $conn->error . '</div>';
    }
}
?>
            <script>
